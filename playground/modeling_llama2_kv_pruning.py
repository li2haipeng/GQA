...
class FuesedLlamaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper"""
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        # self.self_attn = FuesedLlamaAttention(config=config)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # print(len(all_self_attns), all_self_attns[1].size())
        import pandas as pd
        for idx, a in enumerate(all_self_attns):
            a = torch.squeeze(a).cpu()
            if len(a.shape) == 2:
                df = pd.DataFrame(a)
                df.to_csv("results/layer_{}_seq_{}.csv".format(idx, a.shape[-1]), header=False)
