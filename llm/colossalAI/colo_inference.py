...
def inference():
    ...
    if inference_args.override_checkpoint is not None:
        ...
        for i in instruction:
            inputs.append(tokenizer(generate_prompt(i, None), return_tensors="pt").inpu)
        # inputs = tokenizer(generate_prompt(
        #     instruction, None), return_tensors="pt")
        # print(inputs['input_ids'].shape)
        for i in inputs:
            dist.broadcast(i['input_ids'].cuda(), src=0)
        if dist.get_rank() == 0:
            ...
            print("time: {:.2f}".format(generated_tokens[0].shape[0] / (time.time()-start)))
            ...
            model.generate(input_ids=inputs["input_ids"].cuda(),
                            ...
                            return_dict_in_generate=True,
                            output_scores=True)
        # print("time: {:.2f}".format((time.time()-start)/generated_tokens[0].shape[0]))
        start = time.time()
