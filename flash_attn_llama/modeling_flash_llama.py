...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        print("weight", attn_weights.size())
...
class FlashLlamaAttention(nn.Module):
    """Flash Attention with GQA, need to stack kv first"""
...
class BifurcatedLlamaAttention(nn.Module):
    """Bifurcated attention from sciense team"""
    ...
    def forward(
        ...
        if past_key_value is not None:
            # reuse k, v, self_attention, kv_h == h after repeat_kv
            # print("1", past_key_value[0].shape, past_key_value[1].shape)
            # attn_weights_context = torch.einsum("bhnk, ihkm -> bhnm", query_states, past_key_value[0].transpose(-2, -1))
            # attn_weights_incremental = torch.einsum("bhnk, bhkm -> bhnm", query_states, key_states.transpose(-2, -1))

            attn_weights_context = torch.matmul(query_states, past_key_value[0].transpose(2, 3))
            ...
            # print("1", attn_weights_context.shape, attn_weights_incremental.shape, attn_weights.shape)
            # key_states = torch.cat([past_key_value[0], key_states], dim=2)
            # value_states = torch.cat([past_key_value[1], value_states], dim=2)
        ...
        if past_key_value is not None:
            # print("1", past_key_value[0].shape, past_key_value[1].shape)
            context_past_value_len = past_key_value[1].size()[-2]
            # attn_output_context = torch.einsum("bhnm, ihmv -> bhnv", attn_weights[:, :, :, :context_past_value_len], past_key_value[1])
            # attn_out_incremental = torch.einsum("bhnm, bhmv -> bhnv", attn_weights[:, :, :, context_past_value_len:], value_states)
            attn_output_context = torch.matmul(attn_weights[:, :, :, :context_past_value_len], past_key_value[1])
        ...
        if past_key_value is not None:
            past_key = torch.cat([past_key_value[0], key_states], dim=2)
            past_value = torch.cat([past_key_value[1], value_states], dim=2)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # with open('/home/ubuntu/GQA/flash_attn_llama/mem.txt', 'a') as f:
